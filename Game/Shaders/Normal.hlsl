...
VOut VS(VIn vIn)
{
	...
	output.normal = mul(float4(vIn.normal, 0), c_modelToWorld).xyz;
	output.tangent = mul(float4(vIn.tangent, 0), c_modelToWorld);
...
float4 PS(VOut pIn) : SV_TARGET
{
	...
	N = NormalTexture.Sample(DefaultSampler, pIn.text);
	N = 2.0 * N - 1.0;
	N = mul(N, TBN);
	N = normalize(N);
